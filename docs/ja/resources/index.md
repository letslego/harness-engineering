...
- [`templates/`](./templates/): 実際のリポジトリにコピーするテンプレート
- [`reference/`](./reference/): 方法メモ、起動フロー、失敗モードのマップ
- [`openai-advanced/`](./openai-advanced/): 高度なリポジトリ雛形、system-of-record ドキュメント、agent-first ガバナンステンプレート
...
リポジトリが複数ドメイン、アクティブな計画、品質スコア、信頼性ポリシーを持つ長期運用システムに育ったら、最小パックを無理に拡張するのではなく、[`openai-advanced/`](./openai-advanced/) パックへ移行してください。
